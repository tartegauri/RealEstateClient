...
const CreateListing = () => {
  ...
  const [imageUplaodError, setImageUplaodError] = useState(false);
  ...
  const handleImageSubmit = (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          setImageUplaodError("Iamge upload failed (2 mb max per iamge)");
          setUploading(fasle);
        });
    } else {
      setImageUplaodError("you can only upload 6 iamges per listing");
      setUploading(false);
    }
  };
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          {formData.imageUrls.length > 0 &&
            formData.imageUrls.map((url, index) => (
              <div
                key={url}
                className="flex justify-between p-3 boader items-center "
              >
                <img
                  src={url}
                  alt="listing image"
                ...
                <button
                  ...
                  className="p-3 text-red-900 rounded-lg uppercase hover:opacity-90"
                >
                  Delete
                </button>
              </div>
